...
import com.zngcxhy.warehousemanagementsys.repository.entities.PurchaseOrderDetaiEntityMapper;
...
/**
* auto generate PurchaseOrderDetaiEntity ServiceImpl
*
* @author sys
*/
@Service
public class PurchaseOrderDetaiEntityService extends AbstractService {
    @Resource
    private PurchaseOrderDetaiEntityMapper mapper;
    @Resource
    ...
    public PurchaseOrderDetaiEntityService() {
        ...
        entityFieldNameTitleMap.put("quantity", "采购数量");
        entityFieldTitleNameMap.put("采购数量", "quantity");
        ...
        entityFieldNameTitleMap.put("totalAmount", "总金额");
        entityFieldTitleNameMap.put("总金额", "totalAmount");
        entityFiledColumnNameMap.put("totalAmount", "total_amount");
        entityFieldNameTitleMap.put("productID", "产品ID");
        entityFieldTitleNameMap.put("产品ID", "productID");
        entityFiledColumnNameMap.put("productID", "product_i_d");
        entityFieldNameTitleMap.put("productname", "产品名称");
        entityFieldTitleNameMap.put("产品名称", "productname");
        entityFiledColumnNameMap.put("productname", "productname");
    }
    ...
    **/
    ...
    public PurchaseOrderDetaiEntity create(PurchaseOrderDetaiEntity entity) {
        if (null == entity) {
        ...
        entity.setQuantity(getOrDefault(entity.getQuantity(), 0L));
        entity.setUnitPrice(getOrDefault(entity.getUnitPrice(), BigDecimal.ZERO));
        entity.setTotalAmount(getOrDefault(entity.getTotalAmount(), BigDecimal.ZERO));
        entity.setProductID(getOrDefault(entity.getProductID(), 0L));
        entity.setProductname(getOrDefault(entity.getProductname(), ""));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<PurchaseOrderDetaiEntity> batchCreate(List<PurchaseOrderDetaiEntity> entities) {
        ...
        // fill default value
        for (PurchaseOrderDetaiEntity entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            entity.setUpdatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
    ...
    **/
    @Transactional
    public PurchaseOrderDetaiEntity update(PurchaseOrderDetaiEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "PurchaseOrderDetaiEntity");
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<PurchaseOrderDetaiEntity> batchUpdate(List<PurchaseOrderDetaiEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
        // updateFields为null时，默认全量更新
        List<PurchaseOrderDetaiEntity> updateEntities = new ArrayList<>(entities.size());
        for (PurchaseOrderDetaiEntity entity : entities) {
    ...
    **/
    ...
    public long delete( Long id ) { 
        ...
        PurchaseOrderDetaiEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("PurchaseOrderDetai", "purchase_order_detai", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<PurchaseOrderDetaiEntity>  entities = mapper.selectList(inQueryFilter);
        for (PurchaseOrderDetaiEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
     ...
     * auto gen get method
     **/
    public PurchaseOrderDetaiEntity get( Long id ) { 
        if ( id == null ) { 
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
        }
    ...
    **/
    public List<PurchaseOrderDetaiEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<PurchaseOrderDetaiEntity> data = ExcelUtil.read(file.getInputStream(), type, PurchaseOrderDetaiEntity.class, entityFieldTitleNameMap);
            batchCreate(data);
            return "ok";
        } catch (Exception e) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<PurchaseOrderDetaiEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, PurchaseOrderDetaiEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + PurchaseOrderDetaiEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
            throw new HttpCodeException(500, e);
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public PurchaseOrderDetaiEntity createOrUpdate(PurchaseOrderDetaiEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, PurchaseOrderDetaiEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            PurchaseOrderDetaiEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
                entity.setUpdatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(PurchaseOrderDetaiEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
