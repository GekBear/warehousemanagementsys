package com.zngcxhy.warehousemanagementsys.service.entities;
...
import com.zngcxhy.warehousemanagementsys.repository.entities.AfterSalesRequestEntityMapper;
import com.zngcxhy.warehousemanagementsys.util.SnowflakeIdWorker;
import com.zngcxhy.warehousemanagementsys.util.CommonFunctionUtil;
import com.zngcxhy.warehousemanagementsys.service.entities.AbstractService;
import com.zngcxhy.warehousemanagementsys.service.entities.inner.RelationInnerService;
import com.zngcxhy.warehousemanagementsys.service.dto.filter.*;
...
import com.zngcxhy.warehousemanagementsys.util.ExcelUtil;
import com.zngcxhy.warehousemanagementsys.context.UserContext;
/**
* auto generate AfterSalesRequestEntity ServiceImpl
*
* @author sys
*/
@Service
public class AfterSalesRequestEntityService extends AbstractService {
    @Resource
    private AfterSalesRequestEntityMapper mapper;
    @Resource
    ...
    public AfterSalesRequestEntityService() {
        entityFieldNameTitleMap.put("id", "请求编号");
        entityFieldTitleNameMap.put("请求编号", "id");
        ...
        entityFieldNameTitleMap.put("orderID", "采购订单编号");
        entityFieldTitleNameMap.put("采购订单编号", "orderID");
        entityFiledColumnNameMap.put("orderID", "order_i_d");
        entityFieldNameTitleMap.put("supplierID", "供应商ID");
        entityFieldTitleNameMap.put("供应商ID", "supplierID");
        entityFiledColumnNameMap.put("supplierID", "supplier_i_d");
        entityFieldNameTitleMap.put("requestType", "请求类型");
        entityFieldTitleNameMap.put("请求类型", "requestType");
        entityFiledColumnNameMap.put("requestType", "request_type");
        entityFieldNameTitleMap.put("requestDescription", "请求描述");
        entityFieldTitleNameMap.put("请求描述", "requestDescription");
        entityFiledColumnNameMap.put("requestDescription", "request_description");
        entityFieldNameTitleMap.put("requestStatus", "处理状态");
        entityFieldTitleNameMap.put("处理状态", "requestStatus");
        entityFiledColumnNameMap.put("requestStatus", "request_status");
        entityFieldNameTitleMap.put("processresult", "处理结果");
        entityFieldTitleNameMap.put("处理结果", "processresult");
        entityFiledColumnNameMap.put("processresult", "processresult");
    }
    ...
    **/
    ...
    public AfterSalesRequestEntity create(AfterSalesRequestEntity entity) {
        if (null == entity) {
        ...
        entity.setOrderID(getOrDefault(entity.getOrderID(), 0L));
        entity.setSupplierID(getOrDefault(entity.getSupplierID(), 0L));
        entity.setRequestType(getOrDefault(entity.getRequestType(), null));
        entity.setRequestDescription(getOrDefault(entity.getRequestDescription(), ""));
        entity.setRequestStatus(getOrDefault(entity.getRequestStatus(), RequestStatusEnum.FIELD_0));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<AfterSalesRequestEntity> batchCreate(List<AfterSalesRequestEntity> entities) {
        ...
        // fill default value
        for (AfterSalesRequestEntity entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            entity.setUpdatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
    ...
    **/
    @Transactional
    public AfterSalesRequestEntity update(AfterSalesRequestEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "AfterSalesRequestEntity");
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<AfterSalesRequestEntity> batchUpdate(List<AfterSalesRequestEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
        // updateFields为null时，默认全量更新
        List<AfterSalesRequestEntity> updateEntities = new ArrayList<>(entities.size());
        for (AfterSalesRequestEntity entity : entities) {
    ...
    **/
    ...
    public long delete( Long id ) { 
        ...
        AfterSalesRequestEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("AfterSalesRequest", "after_sales_request", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<AfterSalesRequestEntity>  entities = mapper.selectList(inQueryFilter);
        for (AfterSalesRequestEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
     ...
     * auto gen get method
     **/
    public AfterSalesRequestEntity get( Long id ) { 
        if ( id == null ) { 
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
        }
    ...
    **/
    public List<AfterSalesRequestEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<AfterSalesRequestEntity> data = ExcelUtil.read(file.getInputStream(), type, AfterSalesRequestEntity.class, entityFieldTitleNameMap);
            batchCreate(data);
            return "ok";
        } catch (Exception e) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<AfterSalesRequestEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, AfterSalesRequestEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + AfterSalesRequestEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
            throw new HttpCodeException(500, e);
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public AfterSalesRequestEntity createOrUpdate(AfterSalesRequestEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, AfterSalesRequestEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            AfterSalesRequestEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
                entity.setUpdatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(AfterSalesRequestEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
