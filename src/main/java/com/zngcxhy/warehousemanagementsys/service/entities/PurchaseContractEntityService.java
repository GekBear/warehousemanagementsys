...
import com.zngcxhy.warehousemanagementsys.repository.entities.PurchaseContractEntityMapper;
...
/**
* auto generate PurchaseContractEntity ServiceImpl
*
* @author sys
*/
@Service
public class PurchaseContractEntityService extends AbstractService {
    @Resource
    private PurchaseContractEntityMapper mapper;
    @Resource
    ...
    public PurchaseContractEntityService() {
        entityFieldNameTitleMap.put("id", "合同编号");
        entityFieldTitleNameMap.put("合同编号", "id");
        ...
        entityFieldNameTitleMap.put("contractAmount", "合同金额");
        entityFieldTitleNameMap.put("合同金额", "contractAmount");
        entityFiledColumnNameMap.put("contractAmount", "contract_amount");
        entityFieldNameTitleMap.put("contractStatus", "合同状态");
        entityFieldTitleNameMap.put("合同状态", "contractStatus");
        entityFiledColumnNameMap.put("contractStatus", "contract_status");
        entityFieldNameTitleMap.put("contractFile", "合同文件");
        entityFieldTitleNameMap.put("合同文件", "contractFile");
        entityFiledColumnNameMap.put("contractFile", "contract_file");
        entityFieldNameTitleMap.put("contractName", "合同名称");
        entityFieldTitleNameMap.put("合同名称", "contractName");
        entityFiledColumnNameMap.put("contractName", "contract_name");
    }
    ...
    **/
    ...
    public PurchaseContractEntity create(PurchaseContractEntity entity) {
        if (null == entity) {
        ...
        entity.setContractAmount(getOrDefault(entity.getContractAmount(), BigDecimal.ZERO));
        entity.setContractStatus(getOrDefault(entity.getContractStatus(), ContractStatusEnum.FIELD_0));
        entity.setContractFile(getOrDefault(entity.getContractFile(), ""));
        entity.setContractName(getOrDefault(entity.getContractName(), "采购合同"));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<PurchaseContractEntity> batchCreate(List<PurchaseContractEntity> entities) {
        ...
        // fill default value
        for (PurchaseContractEntity entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            entity.setUpdatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
    ...
    **/
    @Transactional
    public PurchaseContractEntity update(PurchaseContractEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "PurchaseContractEntity");
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<PurchaseContractEntity> batchUpdate(List<PurchaseContractEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
        // updateFields为null时，默认全量更新
        List<PurchaseContractEntity> updateEntities = new ArrayList<>(entities.size());
        for (PurchaseContractEntity entity : entities) {
    ...
    **/
    ...
    public long delete( Long id ) { 
        ...
        PurchaseContractEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("PurchaseContract", "purchase_contract", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<PurchaseContractEntity>  entities = mapper.selectList(inQueryFilter);
        for (PurchaseContractEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
     ...
     * auto gen get method
     **/
    public PurchaseContractEntity get( Long id ) { 
        if ( id == null ) { 
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
        }
    ...
    **/
    public List<PurchaseContractEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<PurchaseContractEntity> data = ExcelUtil.read(file.getInputStream(), type, PurchaseContractEntity.class, entityFieldTitleNameMap);
            batchCreate(data);
            return "ok";
        } catch (Exception e) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<PurchaseContractEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, PurchaseContractEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + PurchaseContractEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
            throw new HttpCodeException(500, e);
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public PurchaseContractEntity createOrUpdate(PurchaseContractEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, PurchaseContractEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            PurchaseContractEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
                entity.setUpdatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(PurchaseContractEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
