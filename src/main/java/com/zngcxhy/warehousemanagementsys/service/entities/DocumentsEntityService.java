...
import com.zngcxhy.warehousemanagementsys.repository.entities.DocumentsEntityMapper;
...
/**
* auto generate DocumentsEntity ServiceImpl
*
* @author sys
*/
@Service
public class DocumentsEntityService extends AbstractService {
    @Resource
    private DocumentsEntityMapper mapper;
    @Resource
    ...
    public DocumentsEntityService() {
        ...
        entityFieldNameTitleMap.put("sala", "销售方");
        entityFieldTitleNameMap.put("销售方", "sala");
        entityFiledColumnNameMap.put("sala", "sala");
        entityFieldNameTitleMap.put("orderID", "销售订单号");
        entityFieldTitleNameMap.put("销售订单号", "orderID");
        entityFiledColumnNameMap.put("orderID", "order_i_d");
        entityFieldNameTitleMap.put("tax_rate", "税率");
        entityFieldTitleNameMap.put("税率", "tax_rate");
        entityFiledColumnNameMap.put("tax_rate", "tax_rate");
        entityFieldNameTitleMap.put("price_amout", "价税合计");
        entityFieldTitleNameMap.put("价税合计", "price_amout");
        entityFiledColumnNameMap.put("price_amout", "price_amout");
        entityFieldNameTitleMap.put("pay", "付款方法");
        entityFieldTitleNameMap.put("付款方法", "pay");
        entityFiledColumnNameMap.put("pay", "pay");
        entityFieldNameTitleMap.put("xiaoji", "小计");
        entityFieldTitleNameMap.put("小计", "xiaoji");
        entityFiledColumnNameMap.put("xiaoji", "xiaoji");
        entityFieldNameTitleMap.put("buyer", "购买方");
        entityFieldTitleNameMap.put("购买方", "buyer");
        entityFiledColumnNameMap.put("buyer", "buyer");
        entityFieldNameTitleMap.put("suifri", "税费");
        entityFieldTitleNameMap.put("税费", "suifri");
        entityFiledColumnNameMap.put("suifri", "suifri");
    }
    ...
    **/
    ...
    public DocumentsEntity create(DocumentsEntity entity) {
        if (null == entity) {
        ...
        entity.setSala(getOrDefault(entity.getSala(), 0L));
        entity.setOrderID(getOrDefault(entity.getOrderID(), 0L));
        entity.setTax_rate(getOrDefault(entity.getTax_rate(), 0L));
        entity.setPrice_amout(getOrDefault(entity.getPrice_amout(), BigDecimal.ZERO));
        entity.setPay(getOrDefault(entity.getPay(), ""));
        entity.setXiaoji(getOrDefault(entity.getXiaoji(), BigDecimal.ZERO));
        entity.setBuyer(getOrDefault(entity.getBuyer(), ""));
        entity.setSuifri(getOrDefault(entity.getSuifri(), BigDecimal.ZERO));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<DocumentsEntity> batchCreate(List<DocumentsEntity> entities) {
        ...
        // fill default value
        for (DocumentsEntity entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            entity.setUpdatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
    ...
    **/
    @Transactional
    public DocumentsEntity update(DocumentsEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "DocumentsEntity");
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<DocumentsEntity> batchUpdate(List<DocumentsEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
        // updateFields为null时，默认全量更新
        List<DocumentsEntity> updateEntities = new ArrayList<>(entities.size());
        for (DocumentsEntity entity : entities) {
    ...
    **/
    ...
    public long delete( Long id ) { 
        ...
        DocumentsEntity entity = mapper.selectOne( id ); 
    ...
    **/
     ...
     * auto gen get method
     **/
    public DocumentsEntity get( Long id ) { 
        if ( id == null ) { 
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
        }
    ...
    **/
    public List<DocumentsEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<DocumentsEntity> data = ExcelUtil.read(file.getInputStream(), type, DocumentsEntity.class, entityFieldTitleNameMap);
            batchCreate(data);
            return "ok";
        } catch (Exception e) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<DocumentsEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, DocumentsEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + DocumentsEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
            throw new HttpCodeException(500, e);
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public DocumentsEntity createOrUpdate(DocumentsEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, DocumentsEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            DocumentsEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
                entity.setUpdatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(DocumentsEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
